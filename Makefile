...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pp_cmd_pd_merge_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pp_cmd_pd_merge-*.tar.gz
	echo Create archive \"pp_cmd_pd_merge-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_pd_merge-$(VERSION)-$(BRANCH).tar.gz pd_merge

clean_pack:
	rm -f pp_cmd_pd_merge-*.tar.gz


pp_cmd_pd_merge.tar.gz: build
	cd make_build; tar czf ../pp_cmd_pd_merge.tar.gz pd_merge && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./pd_merge make_build
	cp *.md make_build/pd_merge/
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/pd_merge
...
test:
	docker build --progress=plain -t pp_cmd_merge:$(VERSION) . && docker run -v $$(pwd):/app -it pp_cmd_merge:$(VERSION) python -m unittest tests/test_command.py
